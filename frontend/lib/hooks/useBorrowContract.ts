...
const useBorrowContract = () => {
  ...
  const createBorrowRequest = async (_loanAmount:number,metadata:object) => {
    try{
      ...
      const loanFee = ethers.utils.parseEther(Number(_loanAmount*INTEREST_RATE).toString()) 
      ...
      const tx = await borrowContract.createBorrowerLoan(loanAmount,payoutRate,loanFee,metadataUri)
      await tx.wait();
